...
TASK = {
    ...
    "hint": "追踪 $m$（一阶矩）和 $v$（二阶矩）。$m = \\beta_1 m + (1-\\beta_1)\\nabla$，$v = \\beta_2 v + (1-\\beta_2)\\nabla^2$。偏差修正：$\\hat{m} = m/(1-\\beta_1^t)$，$\\hat{v} = v/(1-\\beta_2^t)$。参数更新：$p \\leftarrow p - \\text{lr} \\cdot \\hat{m} / (\\sqrt{\\hat{v}} + \\epsilon)$。",
    "tests": [
        {
            "name": "参数在 step 后发生变化",
